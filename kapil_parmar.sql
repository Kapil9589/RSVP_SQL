...
select 
  month_num, 
  count(month_num) as number_of_movies 
from 
  cte 
group by 
  month_num 
...
SELECT 
    ...
    MAX(median_rating) AS max_midian_rating
FROM
    ratings;
...
with cte as (
  select 
    count(
      case when languages like '%german%' then m.id end
    ) as total_german_movie_count, 
    sum(
      case when languages like "%german%" then r.total_votes end
    ) as total_vote_count_german, 
    count(
      case when languages like '%italian%' then m.id end
    ) as total_italian_movie_count, 
    sum(
      case when languages like "%italian%" then r.total_votes end
    ) as total_vote_count_italian 
  from 
    movie m 
    join ratings r on m.id = r.movie_id
) 
select 
  round(
    total_vote_count_german / total_german_movie_count, 
    2
  ) as german_votes_per_movie, 
  round(
    total_vote_count_italian / total_italian_movie_count, 
    2
...
SELECT 
    n.name AS actor_name, COUNT(n.id) AS movie_count
FROM
    names n
        JOIN
...
-- Hint: Implement the ROW_NUMBER() function along with ORDER BY to assign ranks to actresses based on average rating and total votes, 
...
with cte as (
  select 
    ...
    case when avg_rating > 8 then 'superhit' when avg_rating between 7 
    and 8 then 'hit' when avg_rating between 5 
    and 7 then 'one-time-watch' else 'flop' end as movie_category, 
...
-- Hint: In the main query, use the SUM() and AVG() window functions to compute the running total duration and moving average duration
--  respectively.
...
select 
  ...
  sum(avg_duration) over(
    order by 
      genre rows between unbounded preceding 
      and current row
  ) as running_total, 
  avg(avg_duration) over(
    order by 
      genre rows between 1 preceding 
      and 1 following
  ) as moving_total 
from 
  cte;
